...
class Utilities:
    ...
    def sanity_check_encoder(self, sentence, block_size):
        ...
        for j, attn_map in enumerate(attn_maps):
            attn_map = attn_map[0]  # select first head
            att_map = attn_map.squeeze(0).detach().cpu().numpy()  # Remove batch dimension and convert to NumPy array

            # print("0:", len(attn_map))
            # print("1:", len(attn_map[0]))
            # print("2:", len(attn_map[0][0]))
            ...
            # Save the plot
            plt.savefig(f"encoder_attention_map_{j + 1}.png")
